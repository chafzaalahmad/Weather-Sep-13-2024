...
final class WeatherViewModel {
        ...
        didSet {
            DispatchQueue.main.async {
                if let location = self.location {
                    ...
                    return self.outputSubject.send(.message(self.noRecordFound))
                }
            }
        }
    }
    ...
    let weatherImageAccessibilityLabel = NSLocalizedString("Current Weather", comment: "")
    let loadingDataActivityAccessibilityLabel = NSLocalizedString("Loading data Activity Indicator View", comment: "")
    let noRecordFound = NSLocalizedString("Sorry! We could not find weather. Please try again later", comment: "")
    ...
    private func process(with location: WeatherLocation) {
        guard let weather = location.weather.first else {
            return outputSubject.send(.message(self.noRecordFound))
        }
        ...
        let minString = NSLocalizedString("Min:", comment: "")
        let maxString = NSLocalizedString("Max:", comment: "")
        ...
        outputSubject.send(
            .weather(
                .init(
                    ...
                    fullDayTemperature: "\(minString) \(formatter.string(from: minTemperature)) \(maxString) \(formatter.string(from: maxTemperature))", description: weather.main, weatherIconUrl: URL(string: "https://openweathermap.org/img/wn/\(weather.icon)@2x.png")
                )
            )
        )
    }
